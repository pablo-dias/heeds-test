...
const Task = ( { task, onDelete, onToggle, edit} ) => {
    ...
    return (
        <div className={`task ${task.reminder ? 'reminder' : ''}`} onDoubleClick={() => onToggle(task.id)}  >
            <h3>{showDiv?<input type='text' className='text-line' value={text} onChange={(e)=>setText(e.target.value)} />: <p>{task.text}</p> } <FaTimes style={{ color: 'red', cursor: 'pointer'}} onClick={() => onDelete(task.id)} /> </h3>
            <p>
